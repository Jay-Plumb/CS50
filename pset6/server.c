...
char* indexes(const char* path)
{
    ...
    // append index.html with path and check if file exists. Do the same with index.html
    char* html = "index.html";
    char* php = "index.php";
    char* htmlPath = malloc(strlen(path) + strlen(html) + 1); // +1 for '/0'
    char* phpPath = malloc(strlen(path) + strlen(php) + 1);
    ...
    // Add / to the end of htmlPath and phpPath
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // Read all available bytes from file
    ...
    for(int c = fgetc(file); c!=EOF; c=fgetc(file)) {
        if (i % 1000 == 0) {
            buffer = realloc(buffer, sizeof(char)*(1000 + i));
        }
        buffer[i] = c;
        i++;
    }
    ...
    *length = i;
    ...
    BYTE buffer[BYTES];
    printf("hi");
    while(fread(buffer, sizeof(BYTE), BYTES, file) != 0 ) {
        printf("hi");
    }
    */
    // printf("load\n");
...
const char* lookup(const char* path)
{
    ...
    // Look for '.' using strrchr
    char* ptr = strrchr(path, '.');
    char* extension = ptr; // Set the suffix to a subset of the path (ignore '.' however so +1)
...
{
    // Should be iterating over line...
    // abs_path must not contain ?, must start with / might have ? so remove. 
    // query must not contain "
    // TODO
  
    // Initial checks on line
   

    char* method = malloc(strlen(line));
    strcpy(method, line);
    method = strtok(method, " ");
    char* requestTarget = strstr(line, " ") + 1; // Get a pointer to the start of the requestTarget
    char* httpVersion = strstr(requestTarget, " ") + 1; // get a pointer to the start of httpVersion
    requestTarget = strtok(requestTarget, " "); // chop off the rest - requestTarget = abs_path + query
    httpVersion = strtok(httpVersion, "\r\n"); // find the '\r\n' and tokenise from here
      
      // Make sure the request line is in the correct format
     ...
     /*
      printf("line: %s [%lu] \n", line, strlen(line)); // length is wrong here
      printf("method: %s [%lu] \n", method, strlen(method));
      printf("requestTarget: %s [%lu] \n", requestTarget, strlen(requestTarget));
      printf("first index: %c \n", requestTarget[0]);
      printf("http status: %s [%lu] \n", httpVersion, strlen(httpVersion)); 
      */
    ...
    if (strstr(requestTarget, "?") != NULL) {
        ...
        char* request = malloc(strlen(requestTarget)); // malloc a coopy of requestTarget so we can MODIFY this for abs_path
        strcpy(request, requestTarget); // actually copy into request memory: note doing request = requestTarget just gives them the same pointer
        // We have ? so lets not copy this icheck50 2015.fall.pset6.server2 server.cnto abs path
        strcpy(abs_path,strtok(request, "?")); // modifies requestTarget - need to change this...
        free(request);
        //strcat(requestTarget, "?"); // add back ? to requestTarget because above line modified requestTarget - what if we have query? lost
        // printf("req tart2: %s [%lu]\n", requestTarget, strlen(requestTarget));
        // query
        char* queryPtr = strstr(requestTarget, "?") + 1; // go to the character after ? to see if anything i there BUG HERE: it should be NULL
     // printf("req tart2: %s [%lu]\n", requestTarget, strlen(requestTarget));
       // printf("%s [%lu] \n", queryPtr, strlen(queryPtr));
        if (strlen(queryPtr) == 0) { // not executed
           // printf("exceuted\n");
            strcpy(query, "\0");
        } else {
           // printf("ths is here\n");
        ...
        // WEe have no ? so copy it straight into abs_path
        strcpy(abs_path, requestTarget);
        strcpy(query, "\0"); // Set the query to 
    }
    
   /*
   printf("query: %s [%lu]\n", query, strlen(query));
   printf("abs_path: %s [%lu]\n", abs_path, strlen(abs_path));
    */
    free(method);
